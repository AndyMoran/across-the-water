...
def fetch_ttf_spot(
    ...
    """TTF natural gas spot price from EMBER bulk CSV. Returns daily EUR/MWh."""
    fetch_ts = _stamp()
    log.info("Fetching TTF spot from EMBER (fetch_ts=%s)", fetch_ts)

    try:
        r  = _get(_EMBER_TTF_URL, timeout=120)
        df = pd.read_csv(io.StringIO(r.text))

        ttf_col  = next((c for c in df.columns if "ttf"  in c.lower() or "gas"  in c.lower()), None)
        date_col = next((c for c in df.columns if "date" in c.lower()), None)

        if ttf_col and date_col:
            df = (
                df[[date_col, ttf_col]]
                .rename(columns={date_col: "date", ttf_col: "ttf_spot_eur_mwh"})
            )
            df["date"]             = pd.to_datetime(df["date"], utc=True)
            df["ttf_is_d1_fallback"] = False
            df["fetch_timestamp"]  = fetch_ts
            df = df.set_index("date").sort_index()
            log.info("TTF spot (EMBER): %d daily rows", len(df))
            return df

    except (requests.HTTPError, requests.ConnectionError, requests.Timeout) as e:
        log.warning("EMBER TTF fetch failed: %s", e)

    log.warning(
        "TTF spot: returning empty DataFrame. "
        "Populate data/ttf_spot.parquet manually (ICE/Refinitiv or EMBER bulk download)."
    )
